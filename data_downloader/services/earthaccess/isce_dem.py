...
class ISCE2AuxData(EarthAccessDownloader):
    ...
    def __init__(self, bbox: Tuple[float, float, float, float], output_dir: str = "."):
        """
    ...
    def download(self) -> List[str]:
        ...
        logger.info(f"Searching for {self.__class__.__name__} with DOI {self.DOI} in bbox {self.bbox}")
        ...
        self.downloaded_files = super().download(results, local_path=self.output_dir)
        return self.downloaded_files

    def merge(self, output_name: str = None, delete_original: bool = False) -> str:
        """
    ...
    def _check_isce2_available(self):
        ...
        if os.path.exists(xml_path):
            ...
            try:
                ...
                if ds:
                    ...
                    logger.warning(f"Could not open {xml_path} with GDAL to generate VRT.")
            except ImportError:
...
class SRTMSWBD(ISCE2AuxData):
    ...
    def merge(self, output_name: str = None, delete_original: bool = False) -> str:
        ...
        try:
            ...
            full_output_path = os.path.join(str(self.output_dir), os.path.basename(output_name))
            st.stitchWbd(
                lat, lon, 
                full_output_path, 
                str(self.output_dir), 
